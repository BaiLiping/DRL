...
class Network(nn.Module):
    def __init__(self,num_features,hidden_sizes,num_classes,feat_dim=10):
        super(Network,self).__init__()
        self.hidden_sizes=[num_features]+hidden_sizes+[num_classes]
        self.layers=[]
        for index, channel_size in enumerate(hidden_sizes):
            self.layers.append(nn.Conv2d(in_channels=self.hidden_sizes[index],
                                         out_channels=self.hidden_sizes[index+1],
                                         kernel_size=3,
                                         stride=2,
                                         bias=False))
    ...
    def forward(self,x,evalMode=False):
        ...
        return closs_output, label_ouput
