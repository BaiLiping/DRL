...
from keras.callbacks import Callback as KerasCallback, CallbackList as KerasCallbackList, TensorBoard
from keras.utils.generic_utils import Progbar

# Note: these callback for log is heavily refered from official keras-rl code
...
class CallbackList(KerasCallbackList):
    ...
    def pass_loss(self, step, logs={}):
        for callback in self.callbacks:
            if callable(getattr(callback, 'pass_loss', None)):
                callback.pass_loss(step, logs=logs)
...
class TrainEpisodeLogger(Callback):
    ...
    def on_train_begin(self, logs):
        ...
        print('Training for {} steps ...'.format(self.params['num_iterations']))
...
class FileLogger(Callback):
    ...
    def on_episode_end(self, episode, logs):
        ...
        # metrics = np.nan    # TODO: CHECK
        # metrics = np.array(self.metrics[episode], dtype=np.float32)
...
# this callback is inherited from keras tensorboard for logging the loss and episode reward to the tensorboard
class TensorboardStepVisualization(TensorBoard):
    def __init__(self, log_dir='./logs',
                 histogram_freq=0,
                 write_graph=True,
                 write_images=False):
        super(TensorboardStepVisualization, self).__init__()
    ...
    def on_episode_end(self, episode, logs={}):
        ...
        for name, value in logs.items():
            if name in ['nb_episode_steps', 'nb_steps']:
                continue
            summary = tf.Summary()
    ...
    def pass_loss(self, step, logs={}):
        ...
        if self.model.training:
            for name, value in logs.items():
                if name in ['action', 'observation', 'reward', 'episode', 'info']:
                    continue
                if name in ['metrics']:
                    value = value[0]
