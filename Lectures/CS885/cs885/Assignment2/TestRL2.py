...
# Test REINFORCE 
policy, policyPa = rlProblem.reinforce(s0=0,initialPolicyParams=np.random.rand(mdp.nActions,mdp.nStates),nEpisodes=1000,nSteps=100)
print( "\nREINFORCE results" )
print( policyPa )
print( policy )

## Test model-based RL
#[V,policy] = rlProblem.modelBasedRL(s0=0,defaultT=np.ones([mdp.nActions,mdp.nStates,mdp.nStates])/mdp.nStates,initialR=np.zeros([mdp.nActions,mdp.nStates]),nEpisodes=400,nSteps=100,epsilon=0.3)
